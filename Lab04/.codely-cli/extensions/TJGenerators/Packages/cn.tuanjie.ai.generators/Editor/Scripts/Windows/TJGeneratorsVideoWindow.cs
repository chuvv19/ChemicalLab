...
namespace TJGenerators
{
    ...
    public class TJGeneratorsVideoWindow : GenerationWindowBase, IGenerationPipelineHost
    {
        ...
        private void DrawHistoryActions()
        {
                ...
                && !string.IsNullOrEmpty(generationHistory[selectedHistoryIndex].modelPath))
            {
                if (GUILayout.Button(TJGeneratorsL10n.L("创建特效播放器"), GUILayout.Height(25)))
                    SetupEffectVideoInScene(generationHistory[selectedHistoryIndex].modelPath);
            }
        ...
        /// <summary>
        /// 特效视频工作流（effect_video_wf）：视频下载后自动创建 ChromaKey 材质。
        ...
        private void TryCreateChromaKeyMaterial(string videoPath, ModelGeneratorBase generator)
        {
            ...
            try
            {
                string outputFolder = Path.GetDirectoryName(videoPath)?.Replace('\\', '/');
                if (!string.IsNullOrEmpty(outputFolder))
                    PathUtils.EnsureAssetFolder(outputFolder);

                var result = GreenScreenVideoPostProcess.ProcessVideo(videoPath, outputFolder);
                if (result.Success)
                {
                    ...
                    SetupEffectVideoInScene(videoPath, result.MaterialPath);
                }
                else
                {
        ...
        /// <summary>
        /// 在场景中自动创建特效视频播放 GameObject：
        /// Quad + VideoPlayer(RenderTexture) + ChromaKey 材质，直接可播放透明特效。
        /// </summary>
        private void SetupEffectVideoInScene(string videoPath, string materialPath = null)
        {
            try
            {
                ...
                if (clip == null)
                {
                    TJLog.LogWarning($"[TJGeneratorsVideo] Cannot setup effect video: clip not found at {videoPath}");
                    return;
                }

                // 如果没传 materialPath，创建或查找 ChromaKey 材质
                ...
                if (mat == null)
                {
                    TJLog.LogWarning($"[TJGeneratorsVideo] Cannot setup effect video: material not found at {materialPath}");
                    return;
                }
                ...
                // 创建 VideoPlayer — RenderTexture 模式
                ...
                // 协程等待 Prepare 完成后再赋 RT，避免空 RT 被 ChromaKey key 掉
                EditorCoroutineUtility.StartCoroutineOwnerless(PrepareAndAssignRT(player, mat, w, h));
                ...
                TJLog.Log($"[TJGeneratorsVideo] Effect video GameObject created: {go.name} (Quad + VideoPlayer + ChromaKey material)");
            }
            catch (Exception e)
            {
                TJLog.LogWarning($"[TJGeneratorsVideo] SetupEffectVideoInScene error: {e.Message}");
            }
        }

        private static IEnumerator PrepareAndAssignRT(VideoPlayer player, Material mat, int w, int h)
        {
