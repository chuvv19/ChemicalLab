...
namespace LiquidVolumeFX {
    ...
    public class LiquidVolumeEditor : Editor {
        ...
        const string LV_SHADER_FEATURE_FOAM = "LIQUID_VOLUME_FOAM";
        ...
        SerializedProperty level, levelMultipler, liquidColor1, liquidColor2, liquidRimColor, liquidRimPower, liquidScale1, liquidScale2, alpha, finalAlphaMultiplier, emissionColor, useLightColor, useLightDirection, directionalLight;
        ...
        SerializedProperty liquidRaySteps, foamRaySteps, fixMesh, pivotOffset, autoCloseMesh, smokeRaySteps, extentsScale, limitVerticalRange, upperLimit, lowerLimit, noiseVariation, foamNoiseTexture, allowViewFromInside;
        ...
        SerializedProperty stencilRef, stencilComp, stencilPass, stencilFail, stencilZFail;
        ...
        bool shaderFeatureScattering, shaderFeatureSmoke, shaderFeatureFoam, shaderFeatureBubbles, shaderFeatureFPRenderTextures, shaderFeatureOrtho, shaderFeatureNoise;
        ...
        void OnEnable () {
            ...
            liquidRimColor = serializedObject.FindProperty("_liquidRimColor");
            liquidRimPower = serializedObject.FindProperty("_liquidRimPower");
            ...
            finalAlphaMultiplier = serializedObject.FindProperty("_finalAlphaMultiplier");
            emissionColor = serializedObject.FindProperty("_emissionColor");
            useLightColor = serializedObject.FindProperty("_useLightColor");
            useLightDirection = serializedObject.FindProperty("_useLightDirection");
            ...
            limitVerticalRange = serializedObject.FindProperty("_limitVerticalRange");
            ...
            foamNoiseTexture = serializedObject.FindProperty("_foamNoiseTexture");
            ...
            stencilRef = serializedObject.FindProperty("_stencilRef");
            stencilComp = serializedObject.FindProperty("_stencilComp");
            stencilPass = serializedObject.FindProperty("_stencilPass");
            stencilFail = serializedObject.FindProperty("_stencilFail");
            stencilZFail = serializedObject.FindProperty("_stencilZFail");
        ...
        void RefreshShaders () {
            ...
            if (path == null) {
                ...
                cachedShaderOptionsPath = path + "/" + LV_SHADER_OPTIONS_FILE;
                cachedScriptOptionsPath = path + "/../../Scripts/" + LV_SCRIPT_OPTIONS_FILE;
            ...
            shaderFeatureFoam = GetShaderOptionState(LV_SHADER_FEATURE_FOAM);
        ...
        public override void OnInspectorGUI () {
            ...
            if (GUILayout.Button("Help", GUILayout.Width(40))) {
                if (!EditorUtility.DisplayDialog("Liquid Volume", "To learn more about a property in this inspector move the mouse over the label for a quick description (tooltip).\n\nPlease check README file in the root of the asset for details and contact support.\n\nIf you like Liquid Volume, please rate it on the Asset Store. For feedback and suggestions visit our support forum on kronnect.com.", "Close", "Visit Support Forum")) {
                    Application.OpenURL("https://kronnect.com/docs/liquid-volume/");
                }
            }
            ...
            EditorGUILayout.PropertyField(alpha, new GUIContent("Global Density", "Global density of the liquid, smoke and foam. You can also combine this with the alpha of the liquid, smoke and foam colors."));
            if (EditorGUI.EndChangeCheck()) {
                requireLayersUpdate = true;
            }
            EditorGUILayout.PropertyField(finalAlphaMultiplier, new GUIContent("Final Alpha Multiplier", "Global transparency of the liquid effect applied just before blending to screen."));
            ...
            if (expandSection[LIQUID_SETTINGS]) {
                ...
                if (detailed == DETAIL.Multiple || detailed == DETAIL.MultipleNoFlask) {
                    ...
                    if (shaderFeatureBubbles) {
                        ...
                        if (bubblesAmount.intValue > 0) {
                            ...
                            EditorGUILayout.PropertyField(bubblesSeed, new GUIContent("Seed"));
                            EditorGUILayout.PropertyField(bubblesSizeMin, new GUIContent("Size Min"));
                            EditorGUILayout.PropertyField(bubblesSizeMax, new GUIContent("Size Max"));
                            EditorGUILayout.PropertyField(bubblesScale, new GUIContent("Scale"));
                            EditorGUILayout.PropertyField(bubblesBrightness, new GUIContent("Brightness"));
                            EditorGUILayout.PropertyField(bubblesVerticalSpeed, new GUIContent("Vertical Speed"));
                            EditorGUI.indentLevel--;
                        }
                        if (EditorGUI.EndChangeCheck()) {
                ...
                if (detailed != DETAIL.Simple && detailed != DETAIL.SimpleNoFlask) {
                    EditorGUILayout.PropertyField(liquidRimColor, new GUIContent("Rim Color"));
                    EditorGUILayout.PropertyField(liquidRimPower, new GUIContent("Rim Power"));
                }
                ...
                EditorGUILayout.PropertyField(frecuency, new GUIContent("Frequency", "Frequency of the turbulence. Increase to produce shorter waves."));
                ...
                if (directionalLight.objectReferenceValue != null) {
                    ...
                    EditorGUILayout.PropertyField(useLightDirection, new GUIContent("Use Light Direction", "Uses light direction to simulate day/night cycle"));
                    EditorGUI.indentLevel--;
                }
                ...
                if (!shaderFeatureScattering) {
                    ...
                    if (scatteringEnabled.boolValue) {
                        if (directionalLight.objectReferenceValue == null || !useLightDirection.boolValue) {
                            EditorGUILayout.HelpBox("Assign a directional light and enable 'Use Direction' option.", MessageType.Info);
                            GUI.enabled = false;
                        }
            ...
            if (expandSection[ADVANCED_SETTINGS]) {
                ...
                EditorGUILayout.PropertyField(noiseVariation, new GUIContent("Liquid Noise Variation", "Choose between 3 different 3D textures."));
                EditorGUILayout.PropertyField(foamNoiseTexture, new GUIContent("Foam Noise Texture", "Use an optional foam texture instead of the same liquid noise texture."));
                ...
                EditorGUILayout.PropertyField(limitVerticalRange, new GUIContent("Limit Vertical Range", "Clamps liquid rendering between a given vertical range."));
                if (limitVerticalRange.boolValue) {
                    EditorGUI.indentLevel++;
                ...
                EditorGUILayout.PropertyField(stencilComp, new GUIContent("Stencil Comparison"));
                EditorGUILayout.PropertyField(stencilPass, new GUIContent("Stencil Pass"));
                EditorGUILayout.PropertyField(stencilFail, new GUIContent("Stencil Fail"));
                EditorGUILayout.PropertyField(stencilZFail, new GUIContent("Stencil ZFail"));
                ...
                if (shaders != null && shaders.Length > 0) {
                    ...
                    if (expandSection[SHADER_VARIANTS]) {
                        ...
                        shaderFeatureFoam = EditorGUILayout.Toggle("Foam", shaderFeatureFoam);
        ...
        static string cachedShaderOptionsPath;
        static string cachedScriptOptionsPath;
        ...
        void UpdateShaderFeatures () {
            ...
            SetShaderOptionState(LV_SHADER_FEATURE_FOAM, shaderFeatureFoam);
        ...
        bool GetShaderOptionState (string option) {
            if (cachedShaderOptionsPath == null || !File.Exists(cachedShaderOptionsPath)) {
                Debug.LogError(LV_SHADER_OPTIONS_FILE + " could not be found!");
                return false;
            }

            string[] code = File.ReadAllLines(cachedShaderOptionsPath, System.Text.Encoding.UTF8);
        ...
        void SetShaderOptionState (string option, string filename, bool state) {
            string path = (filename == LV_SCRIPT_OPTIONS_FILE) ? cachedScriptOptionsPath : cachedShaderOptionsPath;
            if (path == null || !File.Exists(path)) {
                Debug.LogError(filename + " could not be found!");
                return;
            }
