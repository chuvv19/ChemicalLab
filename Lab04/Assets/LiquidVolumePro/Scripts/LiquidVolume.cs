...
namespace LiquidVolumeFX {
    ...
    public static class DetailExtensions {
        ...
        public static bool isDefault (this DETAIL detail) {
            return detail == DETAIL.Default || detail == DETAIL.DefaultNoFlask;
        }
    ...
    [ExecuteInEditMode]
    [HelpURL("https://kronnect.com/docs/liquid-volume/")]
    ...
    public partial class LiquidVolume : MonoBehaviour {
        ...
        [SerializeField]
        CompareFunction _stencilComp = CompareFunction.Always;

        public CompareFunction stencilComp {
            get { return _stencilComp; }
            set {
                if (_stencilComp != value) {
                    _stencilComp = value;
                    UpdateMaterialProperties();
                }
            }
        ...
        [SerializeField]
        [ColorUsage(true)]
        [Tooltip("Adds a colored rim effect. Alpha controls the intensity.")]
        Color _liquidRimColor = new Color(0, 1, 0, 0);

        public Color liquidRimColor {
            get { return _liquidRimColor; }
            set {
                if (_liquidRimColor != value) {
                    _liquidRimColor = value;
                    UpdateMaterialProperties();
                }
            }
        }
        ...
        [SerializeField]
        [Range(0.1f, 4f)]
        float _liquidRimPower = 1f;

        public float liquidRimPower {
            get { return _liquidRimPower; }
            set {
                if (_liquidRimPower != value) {
                    _liquidRimPower = value;
                    UpdateMaterialProperties();
                }
            }
        }
        ...
        [SerializeField]
        [Range(0, 1)]
        float _finalAlphaMultiplier = 1f;

        public float finalAlphaMultiplier {
            get { return _finalAlphaMultiplier; }
            set {
                if (_finalAlphaMultiplier != Mathf.Clamp01(value)) {
                    _finalAlphaMultiplier = Mathf.Clamp01(value);
                    UpdateMaterialProperties();
                }
            }
        }
        ...
        [SerializeField]
        Texture3D _foamNoiseTexture;

        public Texture3D foamNoiseTexture {
            get { return _foamNoiseTexture; }
            set {
                if (_foamNoiseTexture != value) {
        ...
        Vector3 rotatedVerticesPosition, rotatedVerticesScale;
        Quaternion rotatedVerticesRotation;
        int[] verticesIndices;
        ...
        readonly static string[] defaultContainerNames = {
            "GLASS",
            "CONTAINER",
            "BOTTLE",
            "POTION",
        ...
        private void OnValidate () {
            _physicsMass = Mathf.Max(0.1f, _physicsMass);
            _physicsAngularDamp = Mathf.Clamp(_physicsAngularDamp, 0, 0.5f);
        }
        ...
        void UpdateBubblesProperties () {
            ...
            float sizeMultiplier = mr != null ? mr.bounds.size.x : 0.5f;
            liqMat.SetVector(ShaderParams.BubblesData, new Vector4(20f / (1f + _bubblesScale * sizeMultiplier), bubblesVerticalSpeedAcum, _bubblesBrightness, 0));
        }
        ...
        void UpdateDirectionalLight () {
            ...
            if (liqMat == null || _directionalLight == null) return;
        ...
        void UpdateMaterialPropertiesNow () {
            ...
            liqMat.SetInt(ShaderParams.StencilRef, _stencilRef);
            liqMat.SetInt(ShaderParams.StencilComp, (int)_stencilComp);
            liqMat.SetInt(ShaderParams.StencilPass, (int)_stencilPass);
            liqMat.SetInt(ShaderParams.StencilFail, (int)_stencilFail);
            liqMat.SetInt(ShaderParams.StencilZFail, (int)_stencilZFail);
            ...
            liqMat.SetColor(ShaderParams.RimColor, _liquidRimColor);
            liqMat.SetFloat(ShaderParams.RimPower, _liquidRimPower);
            liqMat.SetColor(ShaderParams.EmissionColor, _emissionColor);
            liqMat.SetFloat(ShaderParams.FinalAlphaMultiplier, _finalAlphaMultiplier);
            ...
            if (_foamNoiseTexture != null) {
                liqMat.SetTexture(ShaderParams.FoamNoiseTex, _foamNoiseTexture);
            }
            else if (currentNoiseVariation >= 0) {
                liqMat.SetTexture(ShaderParams.FoamNoiseTex, noise3DTex[currentNoiseVariation]);
            }
        ...
        void UpdateLevels () {
            ...
            if (_rotationLevelCompensation != LEVEL_COMPENSATION.None && !_ignoreGravity && thisLevel > 0) {
                ...
                // RotateVertices(); // commented out since rotatedVertices is now updated lazily
                ...
                if (levelMultipled <= 0)
                    thisLevel = -1f; // ensure it's below the flask thickness
            }

            liquidLevelPos = mr.bounds.center.y - extents.y;
            liquidLevelPos += extents.y * 2f * (levelMultipled > 0 ? thisLevel : 0) + rotationAdjustment;
            ...
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_SPHERE);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_CUBE);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_CYLINDER);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_NON_AABB);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_IGNORE_GRAVITY);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_USE_REFRACTION);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_DEPTH_AWARE);
            liqMat.DisableKeyword(ShaderParams.SHADER_KEYWORD_DEPTH_AWARE_CUSTOM_PASS);

            if (_depthAware) {
                liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_DEPTH_AWARE);
                liqMat.SetFloat(ShaderParams.DepthAwareOffset, _depthAwareOffset);
            }

            if (_depthAwareCustomPass) {
                liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_DEPTH_AWARE_CUSTOM_PASS);
            }

            if (_reactToForces && _topology == TOPOLOGY.Sphere) {
                liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_IGNORE_GRAVITY);
            }
            else if (_ignoreGravity) {
                liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_IGNORE_GRAVITY);
            }
            else if (transform.rotation.eulerAngles != Vector3.zero) {
                liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_NON_AABB);
            }
            switch (_topology) {
                case TOPOLOGY.Sphere:
                    liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_SPHERE);
                    break;
                case TOPOLOGY.Cube:
                    liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_CUBE);
                    break;
                case TOPOLOGY.Cylinder:
                    liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_CYLINDER);
                    break;
                default:
                    break;
            }
            if (_refractionBlur && _detail.allowsRefraction()) {
                ...
                liqMat.EnableKeyword(ShaderParams.SHADER_KEYWORD_USE_REFRACTION);
            }
        ...
        void CheckRotatedVertices () {
            if (verticesUnsorted == null) return;
            bool computeRotatedVertices = false;
            if (rotatedVertices == null || rotatedVertices.Length != verticesUnsorted.Length) {
                computeRotatedVertices = true;
            }
            else if (transform.position != rotatedVerticesPosition || transform.rotation != rotatedVerticesRotation || transform.lossyScale != rotatedVerticesScale) {
                computeRotatedVertices = true;
            }
            if (computeRotatedVertices) {
                RotateVertices();
            }
        }
        ...
        void RotateVertices () {
            ...
            rotatedVerticesPosition = transform.position;
            rotatedVerticesRotation = transform.rotation;
            rotatedVerticesScale = transform.lossyScale;
        }
        ...
        public float GetMeshVolumeUnderLevelWSFast (float level) {
            ...
            if (mr == null || verticesIndices == null) return 0;

            CheckRotatedVertices();

            if (rotatedVertices == null) return 0;
        ...
        public void CenterPivot (Vector3 offset, bool closeMesh) {
            ...
            mesh = Instantiate(mesh);
            mesh.name = meshFilter.sharedMesh.name; // keep original name to detect if user assigns a different mesh to meshfilter and discard originalMesh reference
            ...
            for (int k = 0; k < sortedLayersCount; k++) {
                ...
                if (_liquidLayers[i].miscible) {
                    ...
                    while (_liquidLayers[i].miscible && _liquidLayers[i].density == groupDensity) {
                        ...
                        temp.bubblesOpacity += _liquidLayers[i].bubblesOpacity * amount;
                        temp.viscosity += _liquidLayers[i].viscosity * amount;
                        temp.amount += amount;
                    ...
                    for (int l = k; l < j; l++) {
                        ...
                        _liquidLayers[i].mixedViscosity = temp.viscosity / temp.amount;
                        _liquidLayers[i].mixedAmount = Mathf.Clamp01(temp.amount / groupDensity);
                    ...
                    _liquidLayers[i].mixedViscosity = _liquidLayers[i].viscosity;
            ...
            if (Application.isPlaying) {
                for (int j = 0; j < sortedLayersCount; j++) {
                    ...
                    if (_liquidLayers[k].currentBaseLevel != _liquidLayers[k].baseLevel || _liquidLayers[k].currentAmount != _liquidLayers[k].mixedAmount || _liquidLayers[k].currentMurkiness != _liquidLayers[k].mixedMurkiness || _liquidLayers[k].currentBubblesOpacity != _liquidLayers[k].mixedBubblesOpacity || _liquidLayers[k].currentColor != _liquidLayers[k].mixedColor || _liquidLayers[k].currentColor2 != _liquidLayers[k].mixedColor2 || _liquidLayers[k].currentScale != _liquidLayers[k].mixedScale || _liquidLayers[k].currentViscosity != _liquidLayers[k].mixedViscosity) {
                        ...
                        if (Similar(_liquidLayers[k].currentScale, _liquidLayers[k].mixedScale)) {
                            _liquidLayers[k].currentScale = _liquidLayers[k].mixedScale;
                        }
                        else {
                            _liquidLayers[k].currentScale = Mathf.Lerp(_liquidLayers[k].currentScale, _liquidLayers[k].mixedScale, t);
                        }
                        if (Similar(_liquidLayers[k].currentViscosity, _liquidLayers[k].mixedViscosity)) {
                            _liquidLayers[k].currentViscosity = _liquidLayers[k].mixedViscosity;
                        }
                        else {
                            _liquidLayers[k].currentViscosity = Mathf.Lerp(_liquidLayers[k].currentViscosity, _liquidLayers[k].mixedViscosity, t);
                        }
                        changes = true;
                    }
                }
                if (changes) {
                    ...
                    if (_layersAdjustmentCompact) {
                        ...
                        int liquidLayersCount = _liquidLayers.Length;
                        for (int k = 0; k < liquidLayersCount; k++) {
                            // Get the minimum
                            float minCurrentBaseLevel = float.MaxValue;
                            int min = -1;
                            for (int j = 0; j < liquidLayersCount; j++) {
        ...
        void UpdateLayersProperties () {
            ...
            for (int i = 0; i < sortedLayersCount; i++) {
                ...
                float layerScale = _liquidLayers[k].currentScale * 10.0f;
                ...
                float layerTurbulence = _liquidLayers[k].currentViscosity * alpha;
                float layerBubblesOpacity = _liquidLayers[k].currentBubblesOpacity * alpha;
