        ...
        */
		...
		half4 raymarch(float4 vertex, float3 rd, float t0, float t1) {
			...
			half4 sumFoam  = half4(0,0,0,0);
				...
				if (tmax>tmin) {
					...
					BEGIN_LOOP(k,_FoamRaySteps,7)
						...
						float n = saturate(SAMPLE_NOISE_3D(_FoamNoiseTex, (rpos - disp) * _Scale.y ).r + _FoamDensity);
						if (n>h) {
			...
			if (tmax>tmin) {
                ...
                float local_level = _LevelPos - _Center.y;

                float4 disp  = float4(_Turbulence.y, 1.5, _Turbulence.y, 0) * (_MurkinessSpeed * _Size.y);
                disp.y -= local_level - turbulence;
                rpos.y -= local_level - turbulence;
            ...
            half4 ba = SAMPLE_NOISE_UNWRAPPED((xpos - float4(layerTurbulence, 0, layerTurbulence, 0)) * _BubblesData.x - float4(0, _BubblesData.y, 0,0));
            half4 bb = SAMPLE_NOISE_UNWRAPPED((xpos + float4(layerTurbulence, 0, layerTurbulence, 0)) * _BubblesData.x - float4(0.5, _BubblesData.y * 1.5 + 0.5, 0.5, 0));
            half4 b = ba + bb;
